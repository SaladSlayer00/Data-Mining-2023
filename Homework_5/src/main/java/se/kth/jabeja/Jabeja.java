...
public class Jabeja {
  ...
  private boolean exp_annealing = true;
  private final String FLAG = "custom";
  //-------------------------------------------------------------------
  ...
  private void saCoolDown(){
    if(exp_annealing){
      ...
      System.out.println(config.getDelta());
  ...
  public Node findPartner(int nodeId, Integer[] nodes) {
    ...
    for (int node : nodes) {
      ...
      double oldValue = Math.pow(dpp, alpha) + Math.pow(dqq, alpha);  //represents the cost of the current configuration

      // compute dpq and dqp
      int dpq = getDegree(nodep, nodeq.getColor());
      int dqp = getDegree(nodeq, nodep.getColor());
      double newValue = Math.pow(dpq, alpha) + Math.pow(dqp, alpha); //represents the cost of the proposed new configuration

      if (exp_annealing) {
        double a;
        if(FLAG.equals("custom")) {
          a = 1 / (1 + Math.exp(-(newValue-oldValue)/T));
        }
        else {
          a = Math.exp((newValue - oldValue) / T);
        }
        if ((a > Math.random()) && (newValue > highestBenefit) && (newValue != oldValue)) {
          bestPartner = nodeq;
